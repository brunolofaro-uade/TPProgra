...
public class Set implements ISet {
    ...
    public void remove(int a) {
        ...
        if (this.first != null && this.first.getNext() == null) {
            if (this.first.getValue() == a) {
                if (this.first.getRepetitionCount() > 0) {
                    this.first.subtractRepetition();
                    return;
                } else {
                    this.first = null;
                    this.count--;
        ...
        while (candidate != null) {
            if (candidate.getValue() == a) {
                if (candidate.getRepetitionCount() > 0) {
                    candidate.subtractRepetition();
                    return;
                } else {
